...
## Changelog (2026-07-23d) — Récurrence sans doublon + en-têtes de colonnes collants
- **Plus de doublons à l'avenir** (`create_intervention`, server.py) : lorsqu'une maintenance préventive **récurrente** est réalisée, on **met à jour la fiche existante** (statut reste `planifiee`, `date_planifiee` = date réalisation + périodicité, `compteur_declenchement` recalculé si basé heures) au lieu de créer une nouvelle fiche. L'historique reste rattaché à la même maintenance (`maintenance_preventive_id` inchangé). Les maintenances **ponctuelles** (sans périodicité) ou sur équipement **réformé** restent marquées `terminee`. Vérifié curl : nombre de fiches inchangé (221→221), même fiche recalée (2026-06-20 +180j = 2026-12-17), statut planifiee.
- **En-têtes de colonnes collants** (composant partagé `components/ui/table.jsx`) : le conteneur de table a `max-h-[70vh] overflow-auto` et chaque `TableHead` (th) est `sticky top-0 z-20 bg-white`. Les titres de colonnes restent visibles au défilement sur **tous** les tableaux style Excel (Interventions, Maintenance, Contrôles, Équipements, etc.). Les popovers de filtre (Radix Popover, portal) ne sont pas rognés. Vérifié écran : en-tête fixe pendant le scroll.
